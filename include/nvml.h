...
Orlando's NVML Hack reference, refer to https://github.com/llv22/cuda-smi which has been updated to NVML 11.
Purpose of hacking nvml library, the key is to make nccl on macOS working. So orlando spots that we only have to implement 7 necessary methods.
If possible, why not? the remaining 8 -11, just keep it as ncclSuccess

//see: used in init.cc Line795, only have to load libnvidia-ml.so.1, not blocker
ncclResult_t wrapNvmlSymbols(void);

//see: 1 - used in init.cc Line796
ncclResult_t wrapNvmlInit(void);
//see: 2 - used in init.cc Line808
ncclResult_t wrapNvmlShutdown(void);
//see: 3 - used in topo.cc Line574
ncclResult_t wrapNvmlDeviceGetHandleByPciBusId(const char* pciBusId, nvmlDevice_t* device);
//see: 4 - used in topo.cc Line250
ncclResult_t wrapNvmlDeviceGetNvLinkState(nvmlDevice_t device, unsigned int link, nvmlEnableState_t *isActive);
//see: 5 - used in topo.cc Line254
ncclResult_t wrapNvmlDeviceGetNvLinkRemotePciInfo(nvmlDevice_t device, unsigned int link, nvmlPciInfo_t *pci);
//see: 6 - used in topo.cc Line246
ncclResult_t wrapNvmlDeviceGetNvLinkCapability(nvmlDevice_t device, unsigned int link,
                                                   nvmlNvLinkCapability_t capability, unsigned int *capResult);
//see: 7 - used in topo.cc Line229
ncclResult_t wrapNvmlDeviceGetCudaComputeCapability(nvmlDevice_t device, int* major, int* minor);

//see: 8 - not referenced
ncclResult_t wrapNvmlDeviceGetMinorNumber(nvmlDevice_t device, unsigned int* minorNumber);
//see: 9 - not referenced
ncclResult_t wrapNvmlDeviceGetIndex(nvmlDevice_t device, unsigned* index);
//see: 10 - not referenced
